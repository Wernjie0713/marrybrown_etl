"""
Intelligent Excel Comparison Tool for EOD Summary Reports
Compares Portal vs Xilnex EOD exports with payment breakdown
...
# File paths
PORTAL_FILE = r"C:\Users\MIS INTERN\Downloads\EOD_Summary_2025-10-10_to_2025-10-16 (2).xlsx"
XILNEX_FILE = r"C:\Users\MIS INTERN\Downloads\EOD Sales Summary Report_22-10-2025 (Web).xlsx"
...
def normalize_portal_data(file_path):
    ...
    # Read the Portal export (should have standard structure)
    df = pd.read_excel(file_path, sheet_name=0)
    
    # Standardize column names to match Xilnex format for comparison
    column_mapping = {
        'Date': 'Date',
        'Store': 'Store',
        'Sales Type': 'Sale Type',
        'Sales Return Type': 'Sales Return Type',
        'Sales Transaction Qty': 'Transaction Count',
    ...
    # Handle Sales Return Type (convert NaN to empty string for comparison)
    # Add if not exists (older exports may not have this column)
    if 'Sales Return Type' not in normalized.columns:
        normalized['Sales Return Type'] = ''
        print(f"  Added Sales Return Type column (empty) - older export without this field")
    else:
        normalized['Sales Return Type'] = normalized['Sales Return Type'].fillna('')
...
def normalize_xilnex_data(file_path):
    ...
    # Find the header row (contains "Store", "Date", "Sales Type")
    header_row_idx = None
    for i, row in df_raw.iterrows():
        row_str = ' '.join([str(x) for x in row.values if pd.notna(x)])
        if 'Store' in row_str and 'Date' in row_str and 'Sales Type' in row_str:
            header_row_idx = i
    ...
    # Remove rows with NaN dates AFTER forward-fill
    if 'Date' in normalized.columns:
        before_count = len(normalized)
    ...
    # Handle Sales Return Type (convert NaN to empty string)
    if 'Sales Return Type' in normalized.columns:
        normalized['Sales Return Type'] = normalized['Sales Return Type'].fillna('')
    
    # Note: Xilnex doesn't have E-Wallet Amount column, but it's included in Total Sales
    # Add E-Wallet Amount as 0 for comparison (we know Portal is more detailed here)
    if 'E-Wallet Amount' not in normalized.columns:
        normalized['E-Wallet Amount'] = 0.0
        print(f"  Added E-Wallet Amount column (0.0) - Xilnex doesn't have this breakdown")
...
def compare_dataframes(portal_df, xilnex_df, key_columns):
    ...
    # Get numeric columns to compare (excluding E-Wallet since Xilnex doesn't have it)
    ...
    # Exclude E-Wallet from comparison (Portal has it, Xilnex doesn't)
    common_numeric = [col for col in common_numeric if 'E-Wallet' not in col and 'Wallet' not in col]
...
def generate_comparison_report(merged_df, numeric_columns, output_path):
    ...
    detail_cols = ['Date', 'Store', 'Sale Type', 'Sales Return Type', '_merge']
    for col in numeric_columns:
    ...
    unmatched_df = merged_df[merged_df['_merge'] != 'both'][['Date', 'Store', 'Sale Type', 'Sales Return Type', '_merge']].copy()
...
def main():
    ...
    print("="*80)
    print("INTELLIGENT EOD SUMMARY REPORT COMPARISON")
    print("="*80)
    ...
    key_columns = ['Date', 'Store', 'Sale Type', 'Sales Return Type']
    ...
    output_file = f"EOD_Summary_Comparison_Report_{timestamp}.xlsx"
    ...
    print(f"\nNOTE: E-Wallet Amount is NOT compared (Portal has it, Xilnex doesn't)")
    print(f"      Portal is MORE detailed than Xilnex in payment breakdown!")
    print("="*80)
