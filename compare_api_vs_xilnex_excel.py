...
Handles different structures: Xilnex uses grouped/merged rows, ours is flat
...
print("="*80)
print("API ETL vs XILNEX PORTAL - EXCEL COMPARISON")
print("="*80)
print()
...
print("[STEP 1] Reading Xilnex Export (Grouped Structure)")
print("-"*80)
...
# Exclude "Grand Total" and other non-date totals
...
print(f"  Xilnex Total:  RM {xilnex_total_sales:,.2f}")
print(f"  Our Total:     RM {our_total_sales:,.2f}")
print(f"  Difference:    RM {total_diff:,.2f} ({total_diff_pct:.4f}%)")
print()

print(f"Profit Amount:")
print(f"  Xilnex Total:  RM {xilnex_total_profit:,.2f}")
print(f"  Our Total:     RM {our_total_profit:,.2f}")
...
if sales_accuracy >= success_threshold and profit_accuracy >= success_threshold:
    ...
    print("   API ETL is production-ready!")
else:
    print(f"[WARNING] Accuracy below {success_threshold}% threshold")
    print("   Further investigation needed")
    print(f"   Current: Sales {sales_accuracy:.4f}%, Profit {profit_accuracy:.4f}%")
...
output_file = f"API_vs_Xilnex_Comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
