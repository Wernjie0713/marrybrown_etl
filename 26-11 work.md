...
| Check | Result | Notes |
|-------|--------|-------|
| Duplicate TransactionKeys | ✅ None | Primary key is unique |
| Duplicate grain (SaleNumber+ProductKey+PaymentTypeKey) | ✅ None | Grain is correct |
| Multiple payment types per sale | ✅ None | All 257 sales have 1 payment type |
| SaleType filtering | ✅ Correct | Only 'DINE IN' and 'TAKE AWAY' |
| SalesStatus filtering | ✅ Correct | Only 'COMPLETED' |
| IsFOC (Free of Charge) | ✅ None | No FOC items |
...
- Evidence: `SUM(TotalAmount/Quantity)` ≈ Xilnex total
...
| Finding | Status |
|---------|--------|
| Data validation approach established | ✅ Complete |
| JSON export/comparison pipeline working | ✅ Complete |
| Diagnostic scripts created | ✅ Complete |
| Root cause for MB JITRA identified | 🔍 Quantity inflation (3.83x) |
| Root cause for other stores | 🔍 Needs further investigation |
...
After reviewing the API limitations and business urgency with the Industry Coach, we agreed to **pause the optimized Star-Schema + API-based ETL approach** and switch to a **1:1 Xilnex replica plan** for Phase 1 (2024–2025 data only). The goal is to give Marrybrown immediate ownership of the live sales data, even if the schema remains poor in the short term.
...
2. **Daily ETL job at 2 AM** that loads T-0 (yesterday) data and re-checks T-1 (previous day) for late fixes or voided transactions.
3. **Manual rerun command** to backfill a specific day or date range when needed.
4. **APIs** should first replicate Xilnex’s **Sync Sales API** output from our warehouse; once that matches, extend to Daily Sales, EOD Summary, and Product Mix.
